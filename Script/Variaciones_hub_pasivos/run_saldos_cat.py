...
DECLARE @tc FLOAT, @fecha_1 DATE, @ctiempo_1 INT;

SET @fecha_1 = CAST(DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0) AS DATE);
SET @ctiempo_1 = (SELECT ctiempo FROM Tiempo WHERE fecha = @fecha_1);
SET @tc = (SELECT TipoCambio FROM TipoCambio WHERE cTiempo = @ctiempo_1 AND cMoneda = 170);

IF OBJECT_ID('tempdb..#Cat_Clientes') IS NOT NULL DROP TABLE #Cat_Clientes;
...
WITH PrimerDia AS (
    SELECT periodo, codigoclientebanco, MIN(dia) as dia_entrada
    FROM ##final_ope1
    GROUP BY periodo, codigoclientebanco
)
SELECT 
    f.periodo,
    f.codigoclientebanco,
    CASE 
        WHEN SUM(CASE WHEN moneda = 'USD' THEN saldo_inicial * @tc ELSE saldo_inicial END) = 0 THEN 'Saldo = 0'
        WHEN SUM(CASE WHEN moneda = 'USD' THEN saldo_inicial * @tc ELSE saldo_inicial END) < 3000  THEN '0 a 3000'
        WHEN SUM(CASE WHEN moneda = 'USD' THEN saldo_inicial * @tc ELSE saldo_inicial END) < 5000  THEN '3000 a 5000'
        WHEN SUM(CASE WHEN moneda = 'USD' THEN saldo_inicial * @tc ELSE saldo_inicial END) < 10000 THEN '5000 a 10000'
        ELSE '10000 a más'
    END AS categoria_saldo
INTO #Cat_Clientes
FROM ##final_ope1 f
INNER JOIN PrimerDia p ON f.periodo = p.periodo 
    AND f.codigoclientebanco = p.codigoclientebanco 
    AND f.dia = p.dia_entrada
GROUP BY f.periodo, f.codigoclientebanco;
...
SELECT 
    periodo, dia, producto, moneda, agencia, categoria_saldo, cat, 
    SUM(valor) AS valor, 
    Departamento, Agrupacion, patrimonial, inicial_mas_1m, final_mas_1m
INTO ##tmp_saldos_ope2
FROM (
    SELECT 
        x.periodo, x.dia, x.producto, x.moneda, x.agencia, -- Especificamos x.
        ISNULL(c.categoria_saldo, 'Venta Nueva') AS categoria_saldo,
        x.Departamento, x.Agrupacion, x.Patrimonial, x.inicial_mas_1m, x.final_mas_1m,
        SUM(x.saldo_inicial) AS saldo_inicial, SUM(x.saldo_final) AS saldo_final,
        SUM(x.venta) AS venta, SUM(x.venta_renov) AS venta_renov,
        SUM(x.crecimiento) AS crecimiento, SUM(x.decrecimiento) AS decrecimiento,
        SUM(x.cancelacion) AS cancelacion, SUM(x.cancelacion_renov) AS cancelacion_renov
    FROM (
        SELECT 
            periodo, dia, producto, moneda, agencia, Departamento, Agrupacion, Patrimonial, codigoclientebanco,
            ...
            SUM(saldo_inicial) AS saldo_inicial, SUM(saldo_final) AS saldo_final,
        ...
        FROM (
            SELECT a.*, CASE WHEN b.cat IS NULL THEN a.tipo ELSE a.tipo + '_' + b.cat END AS tipo2
            FROM ##final_ope1 a
            LEFT JOIN ##base_dpf_renovaciones_final1 b ON a.numerooperacion=b.numerooperacion AND a.tipo IN ('venta','cancelacion')
        ) a
        GROUP BY producto, moneda, dia, periodo, agencia, Departamento, Agrupacion, Patrimonial, codigoclientebanco
    ) x
    LEFT JOIN #Cat_Clientes c ON x.periodo = c.periodo AND x.codigoclientebanco = c.codigoclientebanco
    -- CORRECCIÓN AQUÍ: Usar x.periodo para evitar la ambigüedad
    GROUP BY x.periodo, x.dia, x.producto, x.moneda, x.agencia, 
             ISNULL(c.categoria_saldo, 'Venta Nueva'),
             x.Departamento, x.Agrupacion, x.Patrimonial, x.inicial_mas_1m, x.final_mas_1m
) a
UNPIVOT (
    valor FOR cat IN ([saldo_inicial],[saldo_final],[venta],[venta_renov],[crecimiento],[decrecimiento],[cancelacion],[cancelacion_renov])
) b
GROUP BY periodo, dia, producto, moneda, agencia, categoria_saldo, cat, Departamento, Agrupacion, patrimonial, inicial_mas_1m, final_mas_1m
...
def exportar_saldos_completo(df, output_dir, file_prefix="archivoSaldos_", 
    ...
    # 4) Consolidado: une todos los .xlsx mensuales (6 dígitos después del prefijo)
    patron = os.path.join(output_dir, f"{file_prefix}[0-9][0-9][0-9][0-9][0-9][0-9].xlsx")
    archivos_mensuales = glob.glob(patron)

    ruta_consolidado_final = None
    if archivos_mensuales:
        print(f"📦 Consolidando {len(archivos_mensuales)} meses encontrados...")
        lista_meses = []
        for f in archivos_mensuales:
            df_mes = pd.read_excel(f, engine='openpyxl')
            lista_meses.append(df_mes)
        df_final = pd.concat(lista_meses, ignore_index=True).drop_duplicates()

        with pd.ExcelWriter(ruta_consolidado, engine='openpyxl') as w:
            df_final.to_excel(w, index=False)

        ruta_consolidado_final = ruta_consolidado
        print(f'📚 Consolidado total generado en: {ruta_consolidado_final}')

    return ruta_diaria, ruta_mensual, ruta_consolidado_final
...
def main():
    ...
    parser.add_argument("--output-dir", default=os.getenv("OUTPUT_DIR_CATEGORIAS", os.getcwd()), help="Carpeta de salida")
    ...
    with pyodbc.connect(conn_str, autocommit=True) as conn:
        ...
        query_final = """
            SELECT 
                ...
                inicial_mas_1m, final_mas_1m,categoria_saldo
    ...
    diario, mensual, consolidado = rutas
    print("\n✅ Proceso completado.")
    ...
    if consolidado:
        ...
        print("   • Consolidado: (aún no se genera; no hay archivos mensuales suficientes)")
