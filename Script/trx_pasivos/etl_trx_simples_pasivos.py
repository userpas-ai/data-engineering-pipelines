...
# Destino (Escritura)
params_destino = urllib.parse.quote_plus(
    "DRIVER={ODBC Driver 17 for SQL Server};"
    "SERVER=SBDBFPR15;"
...
engine_destino = create_engine(f"mssql+pyodbc:///?odbc_connect={params_destino}", fast_executemany=True)
...
-- VARIABLES
---------------------------------------------------------
declare @dia_anterior date, @ctiempo_inicial int;
set @dia_anterior = GETDATE() -1;
--set @dia_anterior = '2026-06-27' 
set @ctiempo_inicial = (select ctiempo from BFP_DATAWAREHOUSE..Tiempo where Fecha = @dia_anterior );

---------------------------------------------------------
-- 1. TABLA trx_simples_pasivos
---------------------------------------------------------
 
select NumeroOperacion,cProducto,CodigoClienteBanco,FechaValor,Narrativa,Importe,case when cMoneda =129 then 'PEN' when cMoneda=170 then 'USD' end moneda,case
...
end canal_trx,case when DebitoCredito=0 then 'SALIDA' else 'ENTRADA' end salida_entrada from BFP_DATAWAREHOUSE..Fact_Sal_MovimientoContable
 
where cTiempo >= @ctiempo_inicial and cProducto in (33374,
33375,
33387) and NUMEROOPERACION >0;
...
def ejecutar_proceso():
    try:
        ...
        # 1. Extracción desde DataWarehouse
        with pyodbc.connect(CONN_STR_ORIGEN) as conn:
            df = pd.read_sql(sql_script, conn)

        # 2. VALIDACIÓN SOLICITADA: Si está vacía, abortamos el proceso
        if df.empty:
            print("❌ TABLA VACÍA: No se encontraron registros. El proceso se detiene aquí.")
            return # Sale de la función y no ejecuta el bloque de carga

        print(f"✅ Datos obtenidos: {len(df)} registros. Iniciando carga...")

        # 4. Inserción en Marketing (Solo si pasó la validación anterior)
        with engine_destino.begin() as connection:
            df.to_sql(
                name='trx_simples_pasivos', 
                con=connection,
                schema='dbo',
                if_exists='append', 
                index=False,        
        ...
        print(f"🚀 [{datetime.now()}] Carga finalizada con éxito en dbo.trx_simples_pasivos.")
